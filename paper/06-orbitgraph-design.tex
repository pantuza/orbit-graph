...
Figure~\ref{fig:constellation-10x10} shows a representative $10{\times}10$ Walker
constellation from our scaling study, with satellites, a ground station, and the
inter-satellite links that form the edges of this graph.
Distributed link-state protocols such as OSPF~\cite{sidhu:ospf:1993} must
discover each mutation through hello timeouts and link-state flooding before forwarding
...
\begin{figure}[!h]
  ...
  \caption{Representative $10{\times}10$ Walker constellation at 550\,km and
    ...
    Belo Horizonte).
    Orange dots: satellites; green triangle: ground station; light blue lines:
...
No OpenFlow switch is required: static routes installed in the kernel
...
\begin{equation}
  ...
  \label{eq:graph}
\end{equation}
where $V$ is the set of satellite and ground-station nodes,
\begin{equation}
...
In practice $|V| \le 102$ in our study and graph computation remains in
the single-digit millisecond range (Section~\ref{sec:experiments}), so the
control-plane bottleneck is dataplane install, not graph search.
...
dataplane push; unchanged entries are skipped.
At the first GSL handover on the $10{\times}10$ grid, this reduces the install
from ${\sim}10{,}504$ routes at initialization to ${\sim}524$ routes at
handover, roughly $20\times$ fewer programming operations while preserving
an identical logical FIB.

On delay-only ticks (matrices refresh every 10\,s as satellites move
...
maps), the controller skips the dataplane entirely, separating monitoring
overhead from routing churn.

\subsection{Make-before-break dataplane install}
\label{sec:mbb}

For each node $s$ with a non-empty delta, OrbitGraph issues a batched install
inside the container: replace or add every route in $\Delta_s^{+}$, then delete
every stale route in $\Delta_s^{-}$.
Ordering is deliberate: replace runs before delete so a destination whose
next-hop changes never lacks a valid route during the transaction.
...
Route keys map each logical destination node $d$ to host prefixes on the
emulator addressing plan and resolve the egress gateway as the neighbor IP on
the ISL or GSL interface toward $\mathrm{nh}_t(s,d)$.
Address caches are refreshed when topology changes (new interfaces appear) but
not on pure delay ticks, avoiding redundant container scans.
...
We report graph computation time separately from dataplane install time: the
former is the intrinsic algorithmic cost of graph-centric SDN; the latter
reflects our container-based harness and would shrink with a production
southbound (gRPC, NETCONF, or OpenFlow) that streams deltas to routing agents.
...
OrbitGraph's proactive handover exploits SDN's global schedule.
When the emulator processes a topology-change block, the loop creates new GSL
networks, invokes proactive handover at time~$t$ (the controller builds
$G_t$ from the post-add delay matrix $D_t$ and installs $\mathrm{FIB}_t$ while
legacy links still exist), removes obsolete GSLs, and finalizes the topology
change (recomputes $\mathrm{FIB}_t$ on the post-delete graph and installs only
if it differs from the proactive snapshot; in our experiments it does not, so
the finalize step is a verification pass with zero additional routes).
...
\subsection{SDN in the LEO context}
...
strictly more information than any single router's local link-state database at the
moment of mutation.
...
OrbitGraph closes the loop: the same graph model drives both analysis
and live kernel forwarding, avoiding the gap between precomputed paths
(Hypatia-style) and distributed protocol behavior.

SDN separates when routes change from when links change.

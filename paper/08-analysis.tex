...
\begin{figure}[!h]
  ...
  \caption{Data-plane outage at the first GSL handover versus constellation
    size ($N$ nodes), OSPF (red, topology-change events) against OrbitGraph
    (blue, proactive handover).
    Each cell shows the distribution over ten repetitions: the shaded violin is
    the estimated density, the inner box marks the median and interquartile
    range, and every dot is one repetition.
    OSPF exhibits wide, multi-second black holes whose severity depends on
    handover geometry; OrbitGraph collapses to a point mass at 0\,s (dots on the
    baseline) on every grid where the first handover completes inside the
    emulation window.}
...
\begin{figure}[!h]
  ...
  \caption{Empirical cumulative distribution of first-handover data-plane
    outage, pooling every repetition across all constellation sizes.
    ...
    OSPF (red) spreads across seconds, with a tail extending beyond 13\,s.}
  \label{fig:outage-ecdf}
\end{figure}

Figure~\ref{fig:outage-ecdf} pools every handover repetition into a single
empirical cumulative distribution function.
The two curves separate completely: OrbitGraph reaches unit probability at 0\,s,
whereas roughly half of OSPF repetitions incur at least several seconds of
outage and the worst reach the multi-second tail visible in
Figure~\ref{fig:outage-handover}.

At the first handover, OSPF data-plane outage is severe but varies
non-monotonically with constellation size, because it depends on the geometry of
which ground-satellite contact is torn down and how far the replacement path
lies from the old one.
It peaks at ${\sim}13.6$\,s on the $7{\times}7$ grid (51 nodes), with
${\sim}5.7$\,s on $6{\times}6$ (38 nodes) and ${\sim}4.5$\,s on $5{\times}5$
(27 nodes); on several larger grids the first handover happens not to disrupt
the active path, and outage stays below one second
(Figure~\ref{fig:outage-handover}).
Where a black hole does occur, its duration corresponds to the interval between
GSL teardown and BIRD/OSPF reconvergence: traffic is forwarded along stale or
missing next-hops until link-state flooding completes~\cite{sidhu:ospf:1993}.
